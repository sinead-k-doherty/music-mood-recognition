...
def create_max_pooling_layer(layer):
    ...
    return tf.nn.max_pool(value=layer, ksize=[1, 2, 2, 1], strides=[1, 2, 3, 1], padding='SAME')
...
def train(num_iteration, session, accuracy, data, batch_size, x, y_true, optimiser, cost, saver):
    ...
    for i in range(total_iterations, total_iterations + num_iteration):
        ...
        if i % int(data.train.num_examples / batch_size) == 0:
            ...
            show_progress(epoch, feed_dict_tr, feed_dict_val, val_loss, session, accuracy)
            # Save model
            saver.save(session, './app/mmr_model/mir-model5')
...
def build_cnn_model():
    ...
    # Get the dataset
    data = dataset.read_train_sets(img_size, classes, data_type="music")
    ...
    layer_fc1 = create_fully_connected_layer(input=layer_pooling,
    ...
    loss_function = tf.nn.softmax_cross_entropy_with_logits(logits=layer_fc2, labels=y_true)
